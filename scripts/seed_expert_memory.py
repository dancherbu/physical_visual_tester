...
KNOWLEDGE_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), "expert_knowledge.json"))
...
def main():
    print("🧠 Starting Expert Knowledge Seeding...")
    ...
    print(f"ℹ️  Found {len(knowledge_base)} scenarios. Embedding and saving...")
    ...
    for scenario in knowledge_base:
        ...
        print(f"\nProcessing Scenario: {description[:50]}...")
        ...
        for item in actions:
            ...
            if goal and action:
                # We embed the "Prompt" that resembles what the Robot will think at runtime.
                # "Goal: <goal>. Screen: <description>"
                prompt = f"Goal: {goal}. Screen: {full_context}. Prerequisites: {prerequisites}"
                
                print(f"   🔹 Embedding Skill: {goal}...", end='')
                ...
                if vector:
                    payload = {
                        ...
                        "source": "expert_system" # Tag as synthetic
                    }
    ...
    print(f"\n🎉 Seeding Complete! Added {total_added} expert skills to Memory.")
